...
app.get("/", function(req, res) {
	Article.find({}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "To begin Please click \"Scrape For Newest Articles\"  and start saving your favorite articles."});
		}
		else{
...
app.get("/saved", function(req, res) {
	Article.find({issaved: true}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "Start saving your favorite articles by clicking \"Save Article\"!"});
		}
		else {
...
app.post("/search", function(req, res) {
	...
	Article.find({$text: {$search: req.body.search, $caseSensitive: false}}, null, {sort: {created: -1}}, function(err, data) {
		console.log(data);
		if (data.length === 0) {
			res.render("placeholder", {message: "The keyword has not been regonized please try another term."});
		}
		else {
